...
import animationData from './lottieFiles/checkGreen';
import starAnimation from './lottieFiles/star';
import TaskPage from './taskPage';
import moment from 'moment';
import { IconButton } from "./CustomComponent";
export default function Task({id,title,category,isCompleted,isImportant,expectedDate,createdAt,isEvent}) {
	...
	return (
		...
		<div style={{background:category.color}} className="task">
			<div onClick={changeStateCompleted} className="task__checkbox">
					{isCompleted?<Lottie direction={isCompleted?1:-1} height={26} width={26} options={defaultOptions}/>:<IconButton
					color="#0CB293"
					customSize="3px"
				>
					<svg
			...
			<div className="task__info">
				<div onClick={()=>setIsOpen(prev=>!prev)} className="task__info--title">{title}</div>
				<div className="task__info--sub">
					<div className="sub task__expectedDate">
						{moment(expectedDate).format("MMMM DD")}
					</div>
					<div className="sub task__category">{category.name}</div>
				</div>
			</div>
			<div className="task__important" onClick={changeStateImportant}>
			{isImportant ?<div className="lottie__star">
					<Lottie direction={isImportant?1:-1} height={26} width={26} options={defaultOptionsStar}/>
			</div>: (
