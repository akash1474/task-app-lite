...
const Calendar: React.FC<Props> = ({
  ...
  const [selectDate, setSelectDate] = useState<number>(
    defaultDate?.getDate() || new Date().getDate()
  );
  date.setDate(1);
        ...
        days.push(
          <div
            ...
            onClick={(e: React.MouseEvent<HTMLDivElement>) => {
              setSelectDate(parseInt((e.target as HTMLDivElement).innerHTML!));
              setIsOpen(false);
            }}
            className="day today"
          >
  ...
  return (
    <>
      {showDate ? (
        ...
        >
          <Icon fill={iconColor || "#333"} className="calendar__icon" />
          <div className="calendar__pickedDate">{generateDateString()}</div>
        </div>
      ) : (
      ...
      {isOpen && (
        <div ref={ref} style={getPosition(float)} id={id} className="calendar">
